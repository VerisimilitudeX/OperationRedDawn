...
public class Cheget {
  ...
  public static boolean checkSuccess(String[] wires) {
    ...
    if (wires[0].equals("red") && wires[NUM_WIRES - 1].equals("red")) {
      for (int i = 1; i < NUM_WIRES - 1; i++) {
        if (!wires[i].equals("blue")) {
          return false; // one of the middle wires is not blue, so return false
        }
      }
      return true; // all the middle wires are blue, so return true
    } else {
      return false; // the first or last wire is not red, so return false
    }
  }
}
